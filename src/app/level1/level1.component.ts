...
export class Level1Component implements OnInit {
  ...
  level1Lift(input: string) {
    if (input == "ga naar lift" || input == "ga naar een lift" || input == "ga naar de lift" || input == "informatie") {
      ...
      if (this.data.inLift) {
        ...
        this.maakRegel("MACHINE", "Proficiat! Je activeerde de lift! De lift gaat nu naar boven, en je komt terecht op het 2de verdiep. Succes!!");
  ...
  level1Bewaker(input: string) {
    if (input == "ga naar bewaker" || input == "ga naar een bewaker" || input == "ga naar de bewaker" || input == "informatie") {
      ...
      if (this.data.rugzak.includes("mentos") && this.data.rugzak.includes("cola")) {
        this.maakRegel("MACHINE", "Amai, azo nen peet! Jawadde dadde! Hij houdt de inkomhal goed in de gaten. Hij zorgt ervoor dat er geen geld wordt gestolen of mensen zomaar de bank binnen lopen. Hier geraak je niet zomaar voorbij hoor! \n\
      COMMANDO'S: \n\
      ...
      this.maakRegel("MACHINE", "Je steekt het volledige pakje mentos in het cola flesje, en zet het net achter de bewaker. Psssssssst! Alles begint te spuiten, een ware fontein!\
      ...
      this.maakRegel("MACHINE", "Bewaker: \"Het toilet is hier naast de lift naar rechts. Goed mikken hé, we zijn hier niet op scoutskamp!\n\
      COMMANDOS:\n\
      - terug");
    }
  ...
  level1Secretaresse(input: string) {
    if (input == "ga naar secretaresse" || input == "ga naar een secretaresse" || input == "ga naar de secretaresse" || input == "informatie") {
      ...
      this.maakRegel("MACHINE", "De secretaresse heeft mooie blond-bruine krullen. Ze heeft een fleurige blouse en een snoepjesketting aan en kauwgomt er op los, een ware streling voor het oog! 1.5 meter afstand houden!\n\
      COMMANDO'S: \n\
        - vraag waar het toilet is\n\
        - verkoop aansteker\n\
        - terug");
      ...
      this.maakRegel("MACHINE", `Secretaresse: "Sorry, ${this.data.voornaam}. Ik kan geen aansteker gebruiken. Ik kan je niet helpen."\n\
      COMMANDO'S: \n\
        - vraag waar het toilet is\n\
        - verkoop aansteker\n\
        - terug`);
  ...
  level1Zetel(input: string) {
    if (input == "ga naar zetel" || input == "ga naar een zetel" || input == "ga naar de zetel" || input == "informatie") {
      ...
      if (!this.data.muntstukGevonden) {
        ...
        this.maakRegel("MACHINE", "Amai! Je vindt zomaar een muntstuk van 2 euro tussen de zetel! It's your lucky day! Je steekt het muntstuk in je rugzak. \n\
      - rugzak\n\
      - terug");
  ...
  level1Automaat(input: string) {
    ...
    if (input == "ga naar automaat" || input == "ga naar een automaat" || input == "ga naar de automaat" || input == "informatie") {
      ...
      this.maakRegel("MACHINE", `Er zit momenteel ${this.data.bedragInAutomaat} euro in de automaat.`);
      ...
      if (this.data.bedragInAutomaat == 0) {
        ...
        this.maakRegel("MACHINE", `Je haalde ${this.data.bedragInAutomaat} euro uit de automaat.`);
  ...
  geefInstructies() {
    ...
    Op dit moment sta je voor een bank. Dit gebouw heeft verschillende verdiepingen. Jij staat nu buiten, voor het gebouw. Door aan dit systeem commando's te geven, kan je zelf beslissen naar waar je gaat en wat je doet. Ook heb je op dit moment een rugzak aan. Daarin zitten spullen die je hoogstwaarschijnlijk kunnen helpen op je missie. Lees alle tekstjes erg goed!\n\n\
     ...
     Alles zal wel duidelijk worden. Zodra je het gebouw binnenstapt, begint jouw klok te lopen! Probeer een snelle tijd neer ze zetten! Succes! \n\n\
    Om het gebouw binnen te stappen: typ: \"ga binnen\".`);
  }

  welkomstBericht() {
    this.maakRegel("", this.scoutsArduHackSpel, "art");
    this.maakRegel("MACHINE", "Welkom op de online escape room van Scouts Ardu. De bedoeling is dat jij onze scouts helpt door te hacken! Volg de vragen, en denk goed na! Zorg dat je de pagina NIET refreshed! Veel escape plezier!!");
